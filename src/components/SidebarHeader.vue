  ...
  <div>
    ...
    >
      ...
      <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200">
        <h2 class="text-lg font-semibold text-center text-gray-800">STORING</h2>
      ...
      <span
        ...
        class="flex items-center cursor-pointer px-4 py-3 border-b border-gray-200"
      >
      <svg v-if="isDownloading" aria-hidden="true" class="w-5 h-5 mr-2 text-gray-200 animate-spin  fill-blue_557" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <p v-if="!isDownloading"  @click="openDownloadConfirm" class="text-base hover:underline font-semibold text-green_379 text-center">
          Download All
        </p>
        <img
        v-if="!isDownloading"
      ...
      <div class="overflow-y-scroll h-full" >
        <transition-group name="sidebar-item"  tag="div">
          <SidebarVideoItem v-for="item in youtubeStore.savedVideos" :key="item.video.id" :data="item" @removeVideoFromStorage="removeVideoFromStorage"/>
        </transition-group>
      </div>
    </div>
    <Modal :show-modal="modal.show" :title="modal.title" @close="modal.show = false">
      ...
      <div class="text-end mt-8">
        <span class="text-base font-bold text-primary cursor-pointer" @click="downloadAll"
          >Download</span
        >
      </div>
    </Modal>
...
const downloadAll =async () => {
 ...
 await Promise.all(youtubeStore.savedVideos.map(async (item) => {
    const title = `${item.video.title} - ${item.video.channel_name}`
    await youtubeStore.getListDownload(item.video.id).then( (item) => {
      const url = selectedType.value==='audio' ? getMaxAudio(item?.formats || []) : getMinVideo(item?.formats || [])
      if (url) {
        const params: IParams = {
          url,
          title
        }
       const videoCurrent = youtubeStore.savedVideos.find(savedVideo => savedVideo.video.id===item?.id)
...
const downloadMedia =  (params: IParams,videoCurrent : ISidebarVideoItem) => {
  youtubeStore
    .downloadMediaFile(params, (progressEvent) => {
...
const removeVideoFromStorage = (videoId : string) =>{
  youtubeStore.savedVideos = youtubeStore.savedVideos.filter(item => item.video.id !== videoId),
