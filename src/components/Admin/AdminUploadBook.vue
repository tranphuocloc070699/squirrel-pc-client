<template>
    <div class="text-start">
        <button class="rounded-3xl px-2 py-1 text-xs text-white bg-red-600" @click="modal.show=true">
      NEW BOOK
    </button>
      <AppModal
        ...
        v-show="modal.show"
      >
       <div class="overflow-y-scroll h-[500px] p-4">
        <input
        ...
        />
        <p class="mt-4">Pdf Total Page</p>
        <input
          class="w-full px-2 py-1 rounded-md outline-none border border-slate-400"
          type="number"
          v-model="book.pdf_total_page"
          placeholder="Pdf Total Paage"
        />
        <p class="mt-4">Audio Duration</p>
        <input
          class="w-full px-2 py-1 rounded-md outline-none border border-slate-400"
          type="number"
          v-model="book.audio_duration"
          placeholder="Audio Duration"
        />

        <p class="mt-4">Country</p>
        <select
          v-model="book.country_code"
        ...
        >
          <option v-for="item in jsonFile" :key="item.code" :value="item.code">
            {{ item.name }}
          </option>
        </select>
        ...
        <div class="text-end mt-10 mb-10">
          <button @click="handleSubmit" class="bg-primary px-3 py-2 text-white text-sm font-medium">
            Submit
          </button>
        </div>
  ...
  import type { IParams, IUpsertBookRequest } from '@/types'
  ...
  const handleSubmit = async () => {
    ...
    const request: IUpsertBookRequest = {
      ...
      author_id: book.value.author_id,
        pdf_total_page:book.value.pdf_total_page,
        audio_duration:book.value.audio_duration
    }
    console.log({request})
